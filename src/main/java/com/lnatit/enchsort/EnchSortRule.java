...
public class EnchSortRule
{
    ...
    public static void sortDefault(List<ITextComponent> toolTip, ItemStack stack)
    {
        int index;
    ...
    public static void sortCompatible(List<ITextComponent> toolTip, ItemStack stack)
    {
        int index;
        ...
        Map<Map.Entry<Enchantment, Integer>, ITextComponent> lineComponent = new LinkedHashMap<>();
        ...
        for (index = 1; index < toolTip.size(); index++)
        {
            ...
            if (line instanceof TranslationTextComponent)
            {
                ...
                {
                    if (((TranslationTextComponent) line).getKey().equals(ench.getKey().getDescriptionId()))
                    {
                        lineIndex.add(index);
            ...
            {
                for (ITextComponent elem : line.getSiblings())
                {
                    if (elem instanceof TranslationTextComponent)
                    {
                        for (Map.Entry<Enchantment, Integer> sench : enchs)
                        {
                            if (((TranslationTextComponent) elem).getKey().equals(sench.getKey().getDescriptionId()))
                            {
                                lineIndex.add(index);
    ...
    static class EnchComparator implements Comparator<Map.Entry<Enchantment, Integer>>
    {
        ...
        {
            int r1 = 0, r2 = 0, ret;
            ResourceLocation e1 = ForgeRegistries.ENCHANTMENTS.getKey(o1.getKey());
            ResourceLocation e2 = ForgeRegistries.ENCHANTMENTS.getKey(o2.getKey());
